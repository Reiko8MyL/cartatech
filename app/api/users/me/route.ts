...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Obtener mazos recientes (hasta 9, los más recientes) con todos los datos necesarios para mostrar banners
    ...
    try {
      // Obtener los mazos más recientes (públicos y privados)
      recentDecks = await prisma.deck.findMany({
        ...
        take: 9, // Mostrar hasta 9 mazos recientes
        select: {
          id: true,
          name: true,
          description: true,
    ...
    // Obtener favoritos recientes (hasta 12) con todos los datos necesarios para mostrar banners
    ...
    try {
      recentFavorites = await prisma.favoriteDeck.findMany({
        ...
        take: 12, // Mostrar hasta 12 favoritos recientes
        include: {
          deck: {
            select: {
              id: true,
  ...
  try {
    ...
    const updatedUser = await prisma.user.update({
      ...
      data: {
        ...
        ...(showFavoriteRaces !== undefined && { showFavoriteRaces }),
        ...(showFavoriteFormat !== undefined && { showFavoriteFormat }),
        ...(showTeam !== undefined && { showTeam }),
        ...(showPreferredStore !== undefined && { showPreferredStore }),
      },
      select: {
        id: true,
        username: true,
